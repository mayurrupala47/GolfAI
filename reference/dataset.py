...
class Video_IterableDataset(IterableDataset):
    ...
    def __process__(self, imgs):
        """ Process the frame sequence using OpenCV for max speed (Phase 4 optimization) """
        if self.bg_mode:
            median_img = self.median
        ...
        for i in range(self.seq_len):
            ...
            if self.bg_mode == 'subtract':
                # Absolute difference with median
                img = np.sum(np.absolute(img.astype(np.int32) - median_img.astype(np.int32)), axis=2).astype('uint8')
                img = cv2.resize(img, (self.WIDTH, self.HEIGHT), interpolation=cv2.INTER_LINEAR)
                img = img.reshape(1, self.HEIGHT, self.WIDTH)
            elif self.bg_mode == 'subtract_concat':
                diff_img = np.sum(np.absolute(img.astype(np.int32) - median_img.astype(np.int32)), axis=2).astype('uint8')
                diff_img = cv2.resize(diff_img, (self.WIDTH, self.HEIGHT), interpolation=cv2.INTER_LINEAR)
                diff_img = diff_img.reshape(1, self.HEIGHT, self.WIDTH)
