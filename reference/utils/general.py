...
class ResumeArgumentParser():
    ...
    def __init__(self, param_dict):
        ...
        self.epochs = param_dict.get('epochs', 3)
...
def write_pred_video(video_file, pred_dict, save_file, traj_len=8, label_df=None):
    ...
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # mp4v works on all platforms incl. Colab
...
def get_rally_median(video_file):
    ...
    print(f"Calculating background for {rally_id} using saved images (fast)...")
    ...
    if len(frames) == 0:
        print(f"Warning: No frames found in {save_dir} to calculate median!")
        return
