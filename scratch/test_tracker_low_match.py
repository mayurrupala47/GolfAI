...
def main():
    ...
    # Try lower matching threshold
    tracker = sv.ByteTrack(
        track_activation_threshold=0.05,
        lost_track_buffer=30,
        minimum_matching_threshold=0.1,  # Lower matching threshold for small objects
        frame_rate=int(fps)
    )
    ...
    while cap.isOpened():
        ...
        if not detections:
            ...
            for det in detections:
                xyxy_list.append([det[0], det[1], det[2], det[3]])
                conf_list.append(det[4])
                cls_list.append(0)
