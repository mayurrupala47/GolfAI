...
def main():
    ...
    parser.add_argument(
        "--detector",
        type=str,
        choices=["classic", "yolo", "yolo-hybrid", "yolo-only", "openvino"],
        default="classic",
        help="Detector to use: 'classic' (OpenCV HSV), 'yolo' (Hybrid COCO), 'yolo-hybrid', 'yolo-only' (Custom YOLO), or 'openvino' (Native OpenVINO GPU)"
    )
    parser.add_argument(
        "--rtsp-transport",
        type=str,
    ...
    parser.add_argument(
        "--strict-course",
        action="store_true",
        help="Strict course mode: requires ball to sit at Tee before enabling stroke tracking."
    )
    ...
    config["strict_course_mode"] = args.strict_course
    if args.strict_course:
        logger.info("[Course Mode] Strict course tracking enabled: Ball MUST sit at Tee before stroke capturing is enabled.")
    ...
    if detector_type == "yolo-hybrid":
        ...
        logger.info("Configuring Native OpenVINO Ball Detector (Intel Iris Xe GPU Accelerated)...")
        from ai.detector import OpenVINOOnlyBallDetector
        detector = OpenVINOOnlyBallDetector(config)
    ...
    tracker = KalmanBallTracker(config, fps=int(fps), frame_width=frame_width, low_fps=args.laptop, resize_width=resize_width)
