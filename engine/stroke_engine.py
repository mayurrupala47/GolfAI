...
class StrokeEngine:
    ...
    def process_frame(self, frame, frame_idx: int, fps: float, timestamp: float) -> Tuple[List[Dict[str, Any]], Dict[int, BallState]]:
        ...
        for track in tracks:
            x1, y1, x2, y2, track_id, color = track
            ...
            if is_new_track:
                is_tee_preregistered = hasattr(self.tracker, "tracks") and \
                    self.tracker.tracks.get(track_id, {}).get("tee_preregistered", False)
                if is_tee_preregistered:
                    logger.info(f"Ball {track_id}: Tee pre-registration active. Watching for ball placement at tee.")
                else:
            ...
            # Store the color in the state machine so we can refer to it later
            self.state_machines[track_id].color = color
 
            # --- First real tee detection: reset motion history to prevent anchor-jump speed spike ---
            # When the ball is first detected at the tee after pre-registration, the position jumps
            # from the pre-registered anchor to the real ball location. Without a reset, this jump
            # creates a false speed spike that pushes the state machine to MOVING before the stroke.
            if hasattr(self.tracker, "tracks") and track_id in self.tracker.tracks:
                if self.tracker.tracks[track_id].pop("first_real_tee_detection", False):
                    logger.info(f"Ball {track_id}: First real tee detection — resetting motion history to prevent false speed spike.")
                    self.motion_analyzer.reset_track(track_id)
                    # Reset only stale motion fields — keep state (STOPPED/READY) and anchor intact
                    sm = self.state_machines[track_id]
                    if not self.config.get("strict_course_mode", False):
                        sm.placed_on_tee = True
                    sm.ready_position = None
                    sm.last_resting_position = None
                    sm.distance_traveled_since_ready = 0.0
                    sm.has_exceeded_stroke_speed = False
                    if hasattr(sm, "stroke_consecutive_frames"):
                        sm.stroke_consecutive_frames = 0

            # --- Check for anchor escape (ball was hit while anchored at tee/resting) ---
            # When the Kalman tracker detects a real ball position far from the resting anchor,
            # it clears the anchor and sets this flag. We handle it like a re-registration so
            # the motion analyzer is reset and the stroke is counted via on_ball_reappeared.
            anchor_escaped = False
            if hasattr(self.tracker, "tracks") and track_id in self.tracker.tracks:
                anchor_escaped = self.tracker.tracks[track_id].pop("anchor_just_escaped", False)

            # 4b. Handle re-registration (track was lost and came back) OR anchor escape (ball was hit)
            if is_reregistered or anchor_escaped:
                sm = self.state_machines[track_id]
                if anchor_escaped:
                    logger.info(f"Ball {track_id} escaped anchor (hit from rest) at ({cx:.1f}, {cy:.1f}).")
                else:
            ...
            state, stroke_detected, was_reset, hole_complete, tee_placed = state_machine.update(metrics)
            ...
            metrics["placed_on_tee"] = state_machine.placed_on_tee
            active_ball_metrics.append(metrics)
            
            if tee_placed:
                stroke_events.append((track_id, state_machine.stroke_count, "tee_placed"))
