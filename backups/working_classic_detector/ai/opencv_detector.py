...
class OpenCVBallDetector(IBallDetector):
    ...
    def _initialize_mask(self, h: int, w: int):
        ...
        # Scale factor relative to 4K calibration source
        scale_x = w / 3840.0
        scale_y = h / 2160.0
        ...
        # 2. Apply Ignore Regions Mask (Holes, Tees, etc.)
                    ...
                    if circularity >= self.circularity_thresh:
                        ...
                        if abs(avg_diam - self.expected_diameter) <= self.diameter_tolerance:
                            ...
                            if roi.size > 0:
                                ...
                                # White: low Saturation and high Value (strict golf ball white)
                                if mean_hsv[1] <= 65 and mean_hsv[2] >= 160:
