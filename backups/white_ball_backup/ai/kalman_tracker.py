...
class KalmanBallTracker(IBallTracker):
    ...
    """
    def __init__(self, config: Dict[str, Any], fps: int = 60, frame_width: int = 3840):
        self.fps = fps
            ...
            try:
                ...
                for region in cal.get("ignore_regions", []):
                    if "tee" in region.get("name", "").lower():
                        ...
                        # Scale factor is based on actual video frame width
                        scale = resize_width / frame_width
                        self.tee_point = (rx * scale, ry * scale)
        ...
        if self.tee_point is not None:
            ...
            self.tracks[1] = {
                ...
                "bbox": (x1, y1, x2, y2),
                "disappeared": 0,
                "last_matched_center": self.tee_point
            }
    ...
    def update(self, detections: List[Tuple[float, float, float, float, float]], frame: np.ndarray = None, track_states: Dict[int, Any] = None) -> List[Tuple[float, float, float, float, int]]:
        ...
        for tid in list(self.tracks.keys()):
            ...
            # and transition it to a stationary anchor to prevent losing the track.
            if state == "MOVING" and self.tracks[tid]["disappeared"] >= 15:
        ...
        # Register/re-register ONLY Ball ID 1 for unmatched detections if not currently tracked
        if 1 not in self.tracks:
            for idx, det in enumerate(input_dets):
                if idx not in used_detections:
                    ...
                    logger.info(f"Registered/Re-registered Ball ID 1 at position ({cx:.1f}, {cy:.1f})")
                    break
