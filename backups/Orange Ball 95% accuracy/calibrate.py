...
class CalibrationUI:
    def __init__(self, video_path, output_json="config/calibration.json"):
        ...
        # Reduced max height to 650px to guarantee 100% visibility on standard screens
        max_display_h = 650
        if self.orig_h > max_display_h:
            self.disp_h = max_display_h
            self.disp_w = int(self.orig_w * (self.disp_h / self.orig_h))
        else:
        ...
        # Scales for converting display coordinates back to high-res coordinates
        self.to_orig_x = self.orig_w / self.disp_w
        self.to_orig_y = self.orig_h / self.disp_h
    ...
    def save_calibration(self):
        ...
        data = {
            "source_resolution": [self.orig_w, self.orig_h],
