...
class IBallDetector(ABC):
    ...
    """
    @abstractmethod
    def detect(self, frame: np.ndarray, hint_center: tuple = None, hint_moving: bool = False) -> List[Tuple[float, float, float, float, float]]:
        """
...
class IBallTracker(ABC):
    ...
    """
    @abstractmethod
    def update(self, detections: List[Tuple[float, float, float, float, float]], frame: np.ndarray = None) -> List[Tuple[float, float, float, float, int]]:
        """
        ...
        Returns:
            List of active tracked objects as (x1, y1, x2, y2, track_id).
        """
        pass
...
class IMqttPublisher(ABC):
    ...
    @abstractmethod
    def publish_stroke(self, camera_id: str, hole: int, ball_id: int, stroke_count: int, timestamp: str) -> None:
        """
    ...
    @abstractmethod
    def publish_hole_complete(self, camera_id: str, hole: int, ball_id: int, stroke_count: int, timestamp: str) -> None:
        """
    ...
    @abstractmethod
    def publish_reset(self, camera_id: str, hole: int, ball_id: int, timestamp: str) -> None:
        """
