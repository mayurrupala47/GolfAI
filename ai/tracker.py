...
class ByteBallTracker(IBallTracker):
    ...
    def update(self, detections: List[Tuple[float, float, float, float, float, str]], frame: np.ndarray = None, track_states: Dict[int, str] = None) -> List[Tuple[float, float, float, float, int, str]]:
        ...
        if len(valid_dets) == 0:
            ...
            return [(box[0], box[1], box[2], box[3], tid, box[4]) for tid, box in self.tracks.items()]
            
        # Parse inputs into centroids
        input_centroids = []
        for det in valid_dets:
            x1, y1, x2, y2, _, color = det
            cx = (x1 + x2) / 2.0
            cy = (y1 + y2) / 2.0
        ...
        if len(self.tracks) == 0:
            if len(input_centroids) > 0:
                ...
                # Store box coordinates and color
                self.tracks[1] = (det[0], det[1], det[2], det[3], det[5])
            ...
            for row, col in zip(rows, cols):
                ...
                tid = track_ids[row]
                det = input_centroids[col][2]
                self.tracks[tid] = (det[0], det[1], det[2], det[3], det[5])
